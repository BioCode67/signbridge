...
| 한국어→글로스 **학습 모델**(재난문자 한정) | 글로스 F1 **55.8** · BLEU-4 **22.2** (통계 사전은 18.7 / **0.0**) |
...
- `t2gs-v2` — 번역 모델 장기 학습(90에폭). v1(40에폭)은 손실이 아직 내려가는 중에
  끝났다. 끝나면 `ml/eval_t2g_onnx.py`로 재고 나은 쪽을 배포한다.
- 진행은 `~/sbruns/ctc-v1.log` · `~/sbruns/t2gs-v2.log`
...
**다음 할 일**
1. CTC 학습 결과 확인 — 낱말 단위에서 **문장 단위**로 넘어가는 단계
2. t2gs-v2 완료 → 두 판을 재서 나은 쪽 배포(`ml/jobs/deploy_t2g.sh`)
