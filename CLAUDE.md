...
```bash
bash ml/jobs/rebuild_data.sh            # 동작 사전 → 번역 사전 → 어순표 → 검사 9종
...
### 손질은 생성기에도 넣어라 — 사전은 손으로 고치지 않는다

`public/data/align.json`은 **생성물이다.** 손으로 고치면 다음 재생성이 조용히
되돌린다. 2026-08-23에 실측으로 확인했다 — 앞선 세션들의 손질 417건이
빌더에 없어서, 사전을 다시 만들자 **회귀 사례 240건 중 59건이 한꺼번에** 깨졌다
(`연락처`가 전화번호0을 잃고 530으로, `말씀`·`성함`·`써`가 통째로 증발).

고칠 곳은 둘이다:

- 근거를 적을 수 있는 교정 → `ml/etl/build_align_dict.py`의 `SEED_OVERRIDES`
- 활용형·파생형처럼 수가 많은 것 → `ml/data/align_fixes.tsv`
  (`낱말<TAB>글로스`, `-`는 사전에서 뺀다는 뜻)

```bash
python3 ml/tools/check_align_fixes.py   # 손질이 배포본에 살아 있는가 (1초)
```

**확인하는 법**: 생성기를 그대로 돌려 결과가 저장소 파일과 같은지 본다.
다르면 저장소 파일은 생성기가 만든 것이 아니다 — 그 차이가 손질이다.

### 글로스 이름을 소리·입모양으로 내보내지 마라

`시:9시`·`날짜:6월23일`·`시간:2시간`은 낱말이 아니라 **전용 동작의 이름**이다.
읽지 못하면 두 곳에서 새 나간다 — 아바타 입이 "시간시간"을 발음하고, 직원이
스피커로 **"시콜론사시 와요"** 를 듣는다. 후자는 **농인이 자기 말이 어떻게
전달됐는지 볼 수 없는 자리**라 눈으로는 영영 못 잡는다.

읽는 함수는 `readableGloss`(`src/sections/sign/mouthing.ts`) **한 곳뿐**이다 —
마우징과 `glossToKorean`이 같은 것을 쓴다.

```bash
node --experimental-strip-types --import ./scripts/ts-register.mjs \
     scripts/check_gloss_reading.mjs   # 이름표 글로스 1,365종을 전부 읽어 본다
```

### 어순표는 재난문자에서 쟀다 — 그 밖에서는 무리로만 쓴다

`public/data/order.json`의 값 분포는 0.1~0.3에 241종, 0.7~0.9에 124종이고
**0.4~0.6은 하나도 없다.** 표가 아는 것은 "앞쪽(지역명)"과 "뒤쪽(당부)"이라는
두 무리뿐이고 그 안의 소수점 차이는 잡음이다. 창구 문장에 그대로 대면:

    비밀번호를 눌러 주세요 → **누르다0** 비밀0 번호0   (번호 0.69 vs 모르는 동사 0.5)
    인터넷이 안 돼요       → **아니다0** 인터넷0 되다1  (0.692 vs 0.758)

`convert(text, domain)`이 이것을 가른다 — `disaster`만 전면 정렬, 나머지는
무리로 가른 뒤 무리 안에서 원래 순서를 지킨다. **학습 모델을 domain으로 가른
것과 같은 판단이다.**
...
**`-지 않도록`도 같은 함정이다** (2026-08-20 발견). 안전 안내에 가장 흔한 말투인데
`않도록`·`않기`가 **제외어**라 조용히 사라지고, `않게`는 통계 잡음(`중요1`)으로 나갔다.

    물이 넘치지 않도록 하세요       → 물 넘치다 하다     ← **넘치라는 말이 된다**
    넘어지지 않도록 손잡이를 잡으세요 → 손잡이 잡다 넘어지다
    다치지 않도록 조심하세요        → 다치다 조심

**`없습니다`도 제외어였다** — `이용할 수 없습니다`가 **`이용`**으로 나가
"이용하세요"가 됐다. 지금은 `없다` 계열도 못박혀 있다.

지금은 이렇게 못박혀 있다 — **금지형과 서술형을 갈라서** 본다:

    금지형  않도록·않게·않기·않으려면  → 하지마1
    서술형  않아요·않습니다·않은·않는   → 아니다1
    있음/없음  없습니다·없어요·없는     → 없다

**부정만 따로 훑는 검사를 만들었다**(`scripts/check_negation.mjs`). 말투 16종을
한 번에 재고, 부정이 **자기 동사 옆에 있는지**까지 본다. 만들자마자
`-지 않아요`가 빠지는 것을 잡았다(`아직 열지 않아요` → `아직 열다`).

**부정만 남고 동사가 사라지는 것**도 같은 부류다(2026-08-23 발견). 피동형이
사전에 없으면 부정은 살아남고 **부정할 대상이 없어진다**:

    문이 안 열려요 → 문 **아니다**      ← "문이 아니다"로 읽힌다
    불이 안 켜져요 → 불 **아니다**

부정 검사가 "부정이 있는가"만 보고 있어 **통과하고 있었다.** 지금은 부정할
동사가 남아 있는지도 본다. `안`·`못`은 동사 **앞**에 오므로 `PRE_NEG`로
뒤에 올 용언과 한 덩어리로 묶는다(`POST_NEG`의 짝이다) — 안 그러면
"인터넷이 안 돼요"가 `**아니다** 인터넷 되다`가 된다.

```bash
node --experimental-strip-types --import ./scripts/ts-register.mjs \
     scripts/check_negation.mjs      # 부정 말투 16종 + 붙은 자리 + 부정할 대상
``` **그리고 부정은
자기 동사와 한 덩어리로 둔다**(`POST_NEG`) — 안 그러면 어순을 바꿀 때 부정이
엉뚱한 동사 옆으로 밀려 `손잡이 잡다 **하지마** 넘어지다`가 되고, 이번엔
"손잡이를 잡지 마라"로 읽힌다.

### `세 분`과 `삼 분`은 다르다 — 수사 종류로 갈린다

한국어는 **사람을 고유어로, 시간을 한자어로** 센다. 이 구별이 없으면 창구에서
"세 분 기다리세요"가 **"3분 기다리세요"** 로 나간다(2026-08-20 실측).

    세 분 · 두 분   고유어 수사 → **명**(사람)
    삼 분 · 10분    한자어·아라비아 → **분**(시간)

`dictSignAgent.ts`의 `pendingNative`/`lastNative`가 이것을 나른다.
숫자 처리를 건드리면 회귀 사례 `세 분이 오셨어요`·`삼 분 기다리세요`가 지킨다.
...
ml/                  ★ 학습 파이프라인 (지금 작업 중인 곳)
  ...
  data/align_fixes.tsv  손으로 고친 번역 — **여기 없으면 재생성이 되돌린다**
...
| 재난 갈래 한국어 이름 | **41/41** (`scripts/check_categories.mjs`) — 행동요령은 **40/41**(나머지 하나 `BANKINGINFORMATION`은 알림이라 비워 두는 것이 맞다) |
| CTC 디코딩 파이썬↔브라우저 일치 | **3,000판 전부** (`python -m ml.tools.ctc_parity`) |
| 오역 회귀 사례 | **282건** (`scripts/translation_cases.json` — 글로스→한국어 17건 포함) |
