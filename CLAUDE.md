...
| 웹 동작 사전 | **12,833종**(일상어 클립 2,963종 포함) |
| 번역 사전 | **230,160낱말**(활용형·조사형 포함, gzip 905KB) |
| 낱말 표현률 재측정(2026-08-18) | 재난문자 96.1% · 창구 **92.9%** · 일상 96.9% · 길찾기 100% · 행동요령 100% |
...
**돌고 있는 것** (2026-08-18)
- `ctc-v1` — **연속 문장 인식**. 학습 135,446 / 검증 6,617클립 · 7.43M.
  지금은 낱말을 하나씩 쌓아야 한다("머리" 멈춤 "어제" 멈춤) — 농인에게는 대화가
  아니라 조작이다. CTC가 되면 이어서 수어해도 문장으로 읽힌다.
...
**주의: 연속 인식은 낱말 8,147종만 안다**(재난안전 말뭉치로만 학습). 고립 단어
모델은 13,576종이다 — `화장실`처럼 iso-v2에만 있는 낱말은 CTC가 못 낸다.
묻기 화면을 이을 때 두 경로를 함께 두거나, CTC를 합본으로 다시 학습해야 한다.

CTC가 끝나면 **브라우저 쪽은 이미 준비되어 있다** —
`src/recognition/ctcRecognizer.ts`(그리디 디코딩) · `ml/jobs/deploy_ctc.sh`(배포).
파이썬↔브라우저 디코딩 일치는 `python -m ml.tools.ctc_parity`로 확인한다.
남은 것은 묻기 화면에서 낱말 쌓기 대신 이 경로를 쓰도록 잇는 일이다.
...
**다음 할 일**
1. CTC 학습 결과 확인 → `bash ml/jobs/deploy_ctc.sh ~/sbruns/ctc-v1` → 묻기 화면에
   잇기. 낱말 단위에서 **문장 단위**로 넘어가는 단계
...
4. 고정 주소로 옮기기 — `deploy/PAAS.md` (Cloudflare Pages 권장). 계정만 있으면 5분
