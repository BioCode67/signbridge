...
| 방향 | 기능 | 수치 |
|---|---|---|
| 텍스트/음성 → 수어 | **임의 문장**을 3D 아바타가 수어로 | 문장 무제한 · 동작 어휘 5,000종 · 실제 재난문자 246건 실측 낱말 표현 93%(문장 중앙값 100%) |
| | 번역 | KoBART 16만 쌍 파인튜닝(서버) + 통계 사전 폴백(브라우저, 서버 불필요) |
| 수어 → 텍스트 | 웹캠 실시간 단어 인식 | 어휘 8,147종 · top-1 77% (수어자 분리) · 추론 18ms |
| | 글로스 → 자연어 문장 복원 | KoBART 역방향 + **왕복 자체검증**(오역 자동 감지) |
| 왕복 | 수어로 질문 → 답을 수어로 | 4-에이전트 Q&A 연동 |

- **수어 이용자 전용 화면** `#/app` — 텍스트 최소·시각 벨·큰 자막. 받기/말하기 두 탭.
- **실시간 관제** — 실제 재난문자 246건이 수신→번역→합성→송출되는 과정을 실측 시간과 함께 표시.
- **정직성 원칙** — 모든 수치는 측정값만. 시뮬레이션(POP 지연 등)은 화면에 그렇게 표기.
  번역이 뜻을 뒤집을 수 있는 경우(왕복검증 저신뢰) 원문 수어를 앞세운다.
