...
> 🆕 **실시간 수어 인식 MVP**(웹캠→랜드마크→GRU 키워드 인식→자막, 4-에이전트, KoGPT2 Q&A)의
> 구현 현황·검증·실행법·로드맵은 **[MVP.md](MVP.md)** 참고.

> 🧪 **AI Hub 실데이터 학습 파이프라인**(KOREN AI Cloud GPU에서 랜드마크 트랜스포머 학습 →
> ONNX로 브라우저 복귀)은 **[ml/README.md](ml/README.md)**,
> KOREN 워크스페이스·볼륨·잡 운영은 **[ml/KOREN_SETUP.md](ml/KOREN_SETUP.md)** 참고.
> 현재 브라우저 기본 모델은 합성 학습본이며, 이 파이프라인이 그것을 실데이터 모델로 교체한다.
