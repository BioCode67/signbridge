...
PARTICLE_RE = re.compile(
    r"(만요|나요|시기|어야|아야|여야|겠고|겠지|겠다|겠는|이므로|으므로|므로|이라서|라서|이지만|지만|으려면|려면|으면서|면서|으로부터|로부터|에서는|에게서|께서는|하시기|하십시오|입니다|습니다|ㅂ니다"
...
SEED_OVERRIDES = {
    ...
    "났나요": "생기다1",
    "났다": "생기다1",
    # ── 2026-08-23 응급·창구 훑기 2차
    # "움직이지 마세요"가 **`하지마1`** 하나로 나갔다 — 무엇을 하지 말라는지가
    # 사라진다. `움직이다` 클립이 없고 `움직`은 `듣다3`(통계 잡음)을 물고 있었다.
    # `이동하다`가 뜻이 가장 가깝다("이동하지 마세요").
    "움직이다": "이동하다0",
    "움직이": "이동하다0",
    "움직": "이동하다0",
    "움직임": "이동하다0",
    # `주실`이 **`줄다`(감소)** 로 나갔다 — `-실`이 `싣다`로 갈리던 것과 같은 함정이다.
    "주실": "주다",
    "주실래요": "주다",
    "결제": "계산",          # 카드1 — 결제 수단이 아니라 행위다
    "결제하다": "계산",
    "잔돈": "돈1",           # 클립 없음 — '돈'으로라도 뜻이 이어진다
    "이해": "이해",           # 1순위가 숫자 글로스 `이`였다
    # `알아듣다` 클립이 없어 "못 알아들었어요"가 `못하다1 알다 들다0`으로 갈렸다.
    "알아듣다": "이해",
    "알아듣": "이해",
    "알아들": "이해",
    # ── 2026-08-23 행동요령·병원 훑기 3차
    # `번지`(주소의 番地) 클립이 "산불이 번지고 있습니다"를 먹었다 — 불이
    # 퍼진다는 뜻이 사라진다. 낱말은 같아도 뜻이 다르다.
    "번지다": "퍼지다0",
    "번지고": "퍼지다0",
    "번지는": "퍼지다0",
    "번져": "퍼지다0",
    "번져서": "퍼지다0",
    "확산": "퍼지다0",
    "휴교": "쉬다1",          # 클립 없음 — "학교 쉬다"로 뜻이 이어진다
    # `낮은`이 **`낮`(대낮)** 으로 나갔다. "낮은 자세로 이동하세요"가
    # "낮(대낮) 자세" 가 된다 — 몸을 낮추라는 안내다.
    "낮은": "낮다0",
    "낮게": "낮다0",
    "낮추": "낮다0",
    "낮춘": "낮다0",
    "진료비": "돈1",          # 클립 없음 — `비`(rain)로 갈리던 자리다
    "치료비": "돈1",
    "바뀌다": "바꾸다0",
    "바뀌": "바꾸다0",
    "바뀌었": "바꾸다0",
    # 존대 어휘 — `주무시다`는 어간이 달라 규칙으로 못 되살린다.
    "주무시다": "자다",
    "주무시": "자다",
    "주무셨": "자다",
    "주무세요": "자다",
    "주무셨어요": "자다",
    # ── 2026-08-23 재난 구호 훑기
    # `지원`이 **`병0`(병)** 으로 나갔다 — "지원금을 신청하세요"가 `병 금 신청`.
    "지원": "돕다1",
    "지원금": "돈1",
    "자원봉사": "돕다1",
    "자원봉사자": "돕다1",
    "봉사": "돕다1",
    # 구조·소방 인력 — 클립이 `구조`·`소방0`만 있어 통째로 사라졌다.
    "구조대": "구조",
    "구조대원": "구조",
    "소방관": "소방0",
    "소방대원": "소방0",
    "소방차": "소방0",
    "이재민": "피해1",        # 클립 없음 — '피해'로라도 뜻이 이어진다
    "급식": "밥",
    "급식소": "밥",
    "주거": "집0",
    # **`주다`와 `줄다`는 존대형에서 같은 표면형을 만든다** — ㄹ 불규칙이라
    # 줄다 → 주시면, 주다 → 주시면. 통계는 `줄다0`(감소)를 골랐는데, 창구에서
    # "글로 적어 주시면"은 언제나 **주다**다(실측: `글 적다 **줄다** 좋다`).
    # `-실`이 `싣다`로 갈리던 것과 같은 부류다.
    "주시면": "주다", "주셔서": "주다", "주시는": "주다", "주십니다": "주다",
    "주셨어요": "주다", "주시겠어요": "주다", "주셔야": "주다", "주십시오": "주다",
    "주시나요": "주다", "주셨습니다": "주다",
    # `나다`(열이 나다)도 같은 자리에서 **날다(fly)** 로 갈렸다.
    "나시면": "생기다1", "나셔서": "생기다1", "나시는": "생기다1",
    # 이 앱에서 가장 중요한 낱말인데 클립 이름이 `수어`뿐이라 사라졌다.
    "수화": "수어0",
    "수화통역": "수어0",
    "선생님": "선생1",
    # **피동형이 없어 부정만 남았다.** "문이 안 열려요" → `문0 **아니다0**`.
    # 부정은 살아남고 **동사가 사라져** "문이 아니다"로 읽힌다 — 부정이 빠지는
    # 것만큼이나 나쁘다(2026-08-23 실측). 피동은 능동 클립으로 옮긴다.
    "열리다": "열다0", "열려": "열다0", "열려요": "열다0", "열린": "열다0",
    "켜지다": "켜다0", "켜져": "켜다0", "켜져요": "켜다0", "켜진": "켜다0",
    "닫히다": "닫다1", "닫혀": "닫다1", "닫혀요": "닫다1",
    "잠기다": "잠그다1", "잠겨": "잠그다1", "잠겨요": "잠그다1",
    "열쇠": "열다0",          # 클립 없음 — '열다'로라도 뜻이 이어진다
}
...
def conjugations(stem_word: str) -> list[str]:
    ...
    forms = [
        ...
        for s in ("고", "지", "게", "며", "면", "니", "세요", "십시오", "시고",
                  ...
                  "는", "던",
                  # -으러/-러 (목적) — 찾으러·받으러·타러. "분실물을 찾으러
                  # 왔어요"가 `오다` 하나로 나갔다(2026-08-23 실측).
                  "러", "으러",
...
def main() -> None:
    ...
    for word, g in overrides.items():
        ...
        # **시드가 용언이면 활용형도 만든다.**
        #
        # 시드 퍼뜨리기는 **이미 사전에 있는 키**만 덮는다. 그런데 말뭉치에 없어서
        # 시드로 넣은 낱말은 활용형도 없다 — `움직이다`를 시드로 넣어도
        # "움직이지 마세요"는 여전히 `하지마1` 하나로 나갔다(실측 2026-08-23).
        # **무엇을 하지 말라는지가 사라진다.**
        if word.endswith("다") and len(word) >= 3:
            for form in conjugations(word[:-1]):
                if form not in table:
                    table[form] = [g]
                    spread += 1
