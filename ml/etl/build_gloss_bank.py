...
def score_segment(pose: np.ndarray, left: np.ndarray, right: np.ndarray,
                  duration: float, augment: bool, studio: bool) -> float:
...
def main() -> None:
    ...
    def handle(name: str, raw: bytes) -> None:
        ...
        for entry in read_glosses(record, ["both", "strong", "weak"]):
            ...
            score = score_segment(seg_pose, seg_left, seg_right, duration, augment, studio)
            ...
            def round_list(arr: np.ndarray | None, width: int) -> list[list[float]]:
                ...
                return np.round(arr[:, :width].astype(float), 1).tolist()
