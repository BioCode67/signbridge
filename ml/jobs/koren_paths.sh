...
#
# 필요한 용량 — **압축을 풀지 않는다**는 전제.
#   원본 zip   형태소 TL 90G + VL 11G + 수어스크립트 0.4G   ≈ 102G
#   팩·사전    프레임당 약 0.5KB, 450시간 분량              ≈ 25G
#   체크포인트 2.8M 파라미터 × 여러 런                       ≈ 5G
#                                            평상시 합계  ≈ 132G
#
# 다만 **내려받는 도중 한때 3배까지 부푼다.** aihubshell이 download.tar를 받아 분할
# 조각으로 풀고 다시 병합해 zip을 복원하기 때문이다(90G 파일 하나에 최대 ~270G).
# 그래서 순간 최대는 **약 320G**로 봐야 한다. 900GiB면 여유가 크지만, 200G 정도로는
# 학습셋을 못 받는다. 받기 전 `sb_require_space`가 이 값을 확인한다.
#
# zip을 다 풀면 여기에 수백 GB가 더 붙는다. 그래서 `ml.etl.aihub_disaster`가
# zip을 연 채로 멤버만 읽도록 만들어 두었다.
