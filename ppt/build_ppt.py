...
footer(s, "중간보고서(8/17) 이후 iso-v2로 교체 — 클래스 8,147→13,576, 같은 기준으로 다시 쟀습니다")
...
log = [("8월 5일", "HPC 이노베이션 허브", "활용 교육 · PBS 분산 작업 구성 습득"),
       ...
       ("10월", "KOREN 실회선·POP (예정)", "다지점 송출 지연 실측 · 현장 시연")]
...
text(s, 1.02, 3.98, 5.1, 0.32, [("계획에 없던 것 — 3단계에서 더 했습니다", 12.5, True, CY_D)])
extra = ["서비스워커 오프라인 동작 — 회선이 끊겨도 열린다",
         "창구 대화 화면 — 직원 음성 ↔ 농인의 답을 음성으로",
         "재난별 행동요령 수어 · 긴급 SOS 전체화면",
         "숫자 수어(수 읽기) · 표현 못 한 낱말은 낱말 카드로",
         "묻기 — 수어 질의응답과 타일 없는 방향 지도"]
...
mentor = [("7월 23일", "KOREN 자원 활용 미팅",
          ...
          ("지도교수 멘토링", "컴퓨터공학",
           "평가 수치의 신뢰성을 확보할 것 — 실데이터 기반 검증,\n실시간성과 경량화",
           "수어자 분리 평가를 표준으로 못박고, 학습에 없던 수어자 13명으로\n이중 엄격 평가를 더했습니다(78.6%). 브라우저 단독 추론으로 경량화."),
          ("지도교수 멘토링", "사회복지학",
           "농인의 한국어 문해력 편차를 고려할 것 — 텍스트 중심 화면을\n지양하고 실제 당사자 검증이 필요",
...
imgfit(s, "web_기대효과.png", 0.72, 1.5, 6.0, 2.30)
text(s, 0.72, 4.02, 6.0, 0.32, [("구현 난이도 — 무엇이 어려웠나", 12.5, True, INK)])
