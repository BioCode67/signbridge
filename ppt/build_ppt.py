...
def setfont(r, size, bold, color):
    """글꼴을 라틴·한글(ea)·기호(cs) 세 곳에 모두 박는다.

    python-pptx의 `font.name`은 <a:latin>만 적는다. 그러면 한글은
    테마의 동아시아 글꼴(맑은 고딕)로 떨어져서, 리브레오피스에서 만든
    PDF와 파워포인트에서 연 화면의 글꼴이 서로 달라진다.
    큰 글씨는 자간을 조금 좁혀 제목이 성글게 보이지 않게 한다.
    """
    ...
    for tag in ("a:ea", "a:cs"):
        el = rPr.find(qn(tag))
        if el is None:
            el = OxmlElement(tag); latin.addnext(el)
        el.set("typeface", FONT)
    spc = -32 if size >= 24 else (-18 if size >= 15 else 0)
    if spc:
        rPr.set("spc", str(spc))
...
def imgcrop(s, name, x, y, w, top=0.0, bot=0.0, border=True):
    """위·아래를 정한 비율만큼 잘라 내고 폭에 맞춰 넣는다.

    imgfit은 상자를 먼저 정하고 가운데를 기준으로 자른다. 그래서
    갈무리 화면의 아래쪽 숫자 띠가 반쯤 잘려 깨져 보였다. 여기서는
    자를 곳을 직접 정하고 높이는 그 결과로 나오게 한다.
    """
    p = ASSETS / name
    if not p.exists():
        rect(s, x, y, w, w * 0.62, fill=LIGHT, line=LINE, lw=1)
        return None
    from PIL import Image
    iw, ih = Image.open(p).size
    h = w * ih * (1.0 - top - bot) / iw
    sh = s.shapes.add_picture(str(p), I(x), I(y), width=I(w))
    sh.crop_top, sh.crop_bottom = top, bot
    sh.top, sh.height = I(y), I(h)
    if border:
        rect(s, x, y, w, h, fill=None, line=LINE, lw=1)
    return sh
...
def footer(s, note=""):
    ...
    text(s, 0.62, H - 0.34, 8.0, 0.25, [("닿다 · SignBridge · 한국수어 양방향 번역", 9.5, False, MUT)])
    if note:
...
# ═══════════════════════════════════════════ 표지
def waves(s, cx, cy):
    """오른쪽 빈 자리에 넣는 송출 무늬 — 어두운 청록 세 겹."""
    for r_, col in ((2.05, C(0x11,0x28,0x30)), (1.45, C(0x14,0x34,0x3E)), (0.88, C(0x17,0x44,0x51))):
        rect(s, cx - r_, cy - r_, r_ * 2, r_ * 2, fill=None, line=col, lw=1.6,
             shape=MSO_SHAPE.OVAL)
    rect(s, cx - 0.11, cy - 0.11, 0.22, 0.22, fill=CY, shape=MSO_SHAPE.OVAL)
...
text(s, 1.3, 4.66, 10.8, 0.35,
     [("SignBridge · 재난 정보를 수어로 전달하고, 농인의 수어를 한국어와 음성으로 되돌립니다", 12.5, False, C(0x9A,0xA6,0xB4))])
rect(s, 1.3, 5.12, 3.2, 0.035, fill=CY)
# 양식 확인용 표기 — 참가분야·팀명은 운영계획 붙임2 학생팀 목록 기준
for i, (k, v) in enumerate([("참가 분야", "N/W 활용 서비스"), ("팀명", "닿다")]):
    x = 1.3 + i * 3.5
...
cols = [
    ("도입", None, "문제와 목표",
     ["03  재난 정보 전달의 공백", "04  시스템 개요"]),
    ("개발과정 적정성", "30점", "당사자와 과정",
     ["05–06  현장 방문과 피드백", "17  일정 준수와 팀 협업", "18  멘토 의견 반영"]),
    ("개발구현 정도", "30점", "핵심 기술과 검증",
     ["07–10  두 방향 기술·학습 데이터", "11–13  정확도·번역 품질·자동 검사", "14–15  당사자가 쓰는 화면"]),
    ("KOREN 활용도", "20점", "KOREN 활용",
     ["16  KOREN 자원 3종", "16  이용 실적·향후 방안"]),
    ("기대효과", "20점", "한계와 기대효과",
...
text(s, 1.02, 5.28, 11.4, 0.85,
     [("오늘 드릴 말씀은 하나입니다", 11.5, True, CY),
      ("한쪽만 되면 방송이고, 양쪽이 되어야 대화입니다. 받는 방향과 묻는 방향을 모두 만들었습니다.\n"
       "그리고 그 판단을 실험실이 아니라 복지관 두 곳에서 당사자에게 직접 확인했습니다.", 12.5, True, C(0xE3,0xE9,0xEF))], spacing=1.4)
footer(s)
...
bullet(s, 0.72, 1.62, 5.9, gap=0.50, items=[
...
rect(s, 0.72, 4.62, 5.9, 2.20, fill=C(0xFF,0xF6,0xE8))
rect(s, 0.72, 4.62, 0.05, 2.20, fill=AMB)
text(s, 1.05, 4.62, 5.35, 2.20,
     [("우리가 푸는 문제", 12, True, AMB),
      ("", 7, False, MUT),
      ("양방향이어야 한다.\n한쪽만 되면 방송이고,\n양쪽이 되어야 대화다.", 15.5, True, INK)],
     anchor=MSO_ANCHOR.MIDDLE, spacing=1.32)
footer(s)

# ═══════════════════════════════════════════ 시스템 개요
s = sld(); header(s, "시스템 개요", "서버 없이 브라우저 안에서 두 방향이 모두 동작합니다")
rect(s, 0.72, 1.50, 11.9, 2.52, fill=LIGHT)
flow = [("재난문자\n· 직원 음성", 0.0), ("글로스 변환\n(3단 폴백)", 2.42), ("3D 아바타\n수어 재생", 4.84)]
for label, dx in flow:
    x = 1.05 + dx
    rect(s, x, 1.78, 2.05, 0.94, fill=CY)
    text(s, x, 1.95, 2.05, 0.7, [(label, 12.5, True, WHITE)], align=PP_ALIGN.CENTER, spacing=1.2)
flow2 = [("웹캠 수어\n랜드마크", 0.0), ("낱말 인식\n13,576종", 2.42), ("한국어 문장\n· 음성 출력", 4.84)]
for label, dx in flow2:
    x = 1.05 + dx
    rect(s, x, 2.80, 2.05, 0.94, fill=C(0x2C,0x7A,0x86))
    text(s, x, 2.97, 2.05, 0.7, [(label, 12.5, True, WHITE)], align=PP_ALIGN.CENTER, spacing=1.2)
text(s, 8.05, 1.88, 4.3, 1.9,
...
# ═══════════════════════════════════════════ 현장 방문 (건물 + 개요)
s = sld(); header(s, "두 곳의 복지관에서 직접 확인했습니다",
...
text(s, 8.35, 1.72, 4.0, 1.9,
     [("숫자로는 알 수 없는 것이 있습니다", 13, True, CY),
      ("뒤에서 인식 정확도와 번역 품질을 숫자로 보여\n드립니다. 그런데 그 숫자는 “낱말이 나갔는가 /\n맞게 나갔는가”까지만 잽니다.", 10.5, False, C(0xC5,0xCE,0xD8)),
      ("농인이 보고 뜻이 통하는지는 다른 질문이라,\n만들자마자 복지관 두 곳으로 가져갔습니다.", 11, True, WHITE)], spacing=1.3)
# 현장 사진 3장
photo_slot(s, 0.72, 4.28, 3.85, 2.05, "수어 이용자에게 앱을 직접 소개", 1)
photo_slot(s, 4.82, 4.28, 3.85, 2.05, "질의응답 화면을 함께 확인", 5)
photo_slot(s, 8.92, 4.28, 3.71, 2.05, "옆에서 조작을 도우며 관찰", 4)
footer(s, "2026년 8월 · 경산 · 대구")

# ═══════════════════════════════════════════ 시연과 피드백 (사진 4)
s = sld(); header(s, "실제 이용을 돕고, 개선점을 받았습니다",
                  "아바타 수어를 함께 보며 무엇이 읽히고 무엇이 안 읽히는지 확인했습니다")
photo_slot(s, 0.72, 1.5, 2.85, 1.95, "아바타 수어를 함께 보며 확인", 2)
photo_slot(s, 3.77, 1.5, 2.85, 1.95, "화면 조작을 안내하며 시연", 3)
photo_slot(s, 6.82, 1.5, 2.85, 1.95, "시스템 구조를 설명", 6)
photo_slot(s, 9.87, 1.5, 2.76, 1.95, "기대효과를 함께 이야기", 7)
...
agents = [("① 재난 판단", "문자에서 재난 종류·심각도·지역을 뽑는다"),
          ("② 수어 변환", "한국어 문장을 글로스 열로 바꾼다 (3단 폴백)"),
...
caption(s, 0.72, 6.36, 2.27, "재난문자가 수어로 나오는 받기 화면")
bullet(s, 3.40, 1.52, 4.70, gap=0.48, items=[
...
caption(s, 8.45, 6.42, 4.18, "사람 번역가 정답과 비교한 번역 품질 · 검증 800문장")
footer(s)
...
steps = [("① 랜드마크 추출", "MediaPipe로 상반신·양손 관절 좌표"),
         ("② 155차원 특징", "어깨 기준 정규화. 학습과 추론이 같은 정의"),
...
text(s, 1.0, 6.00, 11.4, 0.7,
     [("가장 찾기 어려운 실패입니다. 학습 특징과 추론 특징이 어긋나면 “검증 정확도 95%인데 웹캠에선 0%”가 됩니다.", 12, True, INK),
      ("파이썬 학습 코드와 브라우저 추론 코드의 수치를 대조하는 검사를 두어 현재 오차 0을 유지합니다.", 11, False, MUT)], spacing=1.3)
footer(s)

# ═══════════════════════════════════════════ 데이터
s = sld(); header(s, "학습 데이터", "AI Hub 수어 말뭉치를 원본부터 직접 열어 확인하며 가공했습니다")
...
traps = [("확장자가 .zip인데 실제로는 7z", "매직 바이트로 판별. 통짜 압축이라 한 번만 훑으며 처리"),
         ("메타가 중첩되어 있음", "signer가 최상위에 없다. 못 찾으면 수어자 분리가 무력화된다"),
         ("파일명 표기가 문서와 다름", "가이드는 FINSP, 실데이터는 FS. 0건인데 오류도 안 난다"),
         ("좌표 단위가 제각각", "mm·m·픽셀 혼재. 절대 임계값 대신 중앙값 기준으로 판정"),
         ("깊이 추정 실패 프레임", "-25,000,000 같은 값이 섞인다. 정제 없이는 학습이 망가진다")]
...
footer(s, "중간보고서(8/17) 이후 iso-v2로 교체했습니다. 클래스를 8,147에서 13,576으로 늘려 같은 기준으로 다시 쟀습니다")
...
text(s, 7.55, 2.20, 4.85, 1.2,
     [("복합어 처리 버그로", 10.5, False, MUT),
      ("“어디가 아프신지” → 어디 아프다 신다(신발)로", 11.5, True, C(0xB0,0x39,0x39)),
      ("번역되던 동안에도 표현률은 그대로였습니다.\n잘못된 수어는 표현되지 않은 것보다 나쁩니다.\n농인은 그것을 믿기 때문입니다.", 10.3, False, INK)], spacing=1.28)
...
text(s, 1.0, 5.26, 5.75, 1.4,
     [("튜닝에 쓴 문장으로 재지 않습니다", 12.5, True, CY_D),
      ("사전을 고칠 때는 튜닝 집합만 보고, 홀드아웃은 결과 확인에만 씁니다.\n"
       ...
       "창구 대화 홀드아웃은 94.8%입니다. 일반화를 재는 정직한 값입니다.", 10.6, False, INK)], spacing=1.32)
footer(s)
...
# ═══════════════════════════════════════════ 앱 화면
s = sld(); header(s, "당사자가 쓰는 네 개의 탭", "농인이 실제로 쓰는 화면입니다")
tabs = [("web_app_받기.png", "받기", "재난문자가 수어로 나온다", 0.72, 1.45),
        ("web_app_묻기.png", "묻기", "수어로 묻고 수어로 답을 받는다", 6.72, 1.45),
        ("web_app_대화.png", "대화", "창구에서 직원 말과 농인의 답을 잇는다", 0.72, 4.28),
...
caption(s, 0.72, 4.30, 7.0, "묻기 화면. 방향과 거리, 걸리는 시간, 방향 지도, 아바타 수어 응답")
...
footer(s, "장소 목록은 OSM 기반 참고용입니다. 공공데이터 인증키를 받으면 공식 목록으로 교체합니다")
...
res = [("① AI Cloud (CHEETAH)", "학습 전용", CY,
        ...
        "인식·번역 모델 학습과 평가. 에폭당 21~25분에 215만 세그먼트를 처리했습니다."),
...
text(s, 1.02, 6.46, 5.9, 0.4,
     [("MIG는 메모리는 넉넉하고 연산이 부족합니다. 그래서 영상 CNN 대신 랜드마크 트랜스포머를 골랐습니다.", 10, True, INK)])
...
log = [("8월 5일", "HPC 이노베이션 허브", "활용 교육 · PBS 분산 작업 구성 습득"),
       ...
       ("8월 3주", "KOREN AI Cloud", "인식·번역 학습 · 수어자 분리 평가"),
       ("8월 4주", "HPC (예정)", "다각도 48,000클립 분산 변환"),
       ("9월", "KOREN VM (예정)", "상시 데모 서버 운영"),
       ("10월", "KOREN 실회선 (예정)", "NIA POP 다지점 송출 지연 실측")]
...
text(s, 7.70, 6.10, 4.75, 0.75,
     [("왜 KOREN에 맞는 과제인가", 10.5, True, CY),
      ("영상 대신 관절 좌표를 보냅니다. 문장당 약 2.4KB로 영상의 1/2,000입니다.\n"
       "10월 실회선 POP에서 다지점 송출 종단 지연을 실측합니다.", 10, False, C(0xD5,0xDD,0xE5))], spacing=1.3)
footer(s, "심사항목 · KOREN 활용도 20점")

# ═══════════════════════════════════════════ 개발 과정 — 일정과 협업
s = sld(); header(s, "계획대로 왔고, 계획에 없던 것도 했습니다",
                  "중간보고서의 5단계 계획 대비 진행 상황입니다")
steps = [("1단계", "7월", "데모 웹앱 · 아바타 리타게팅\n규칙 기반 변환 MVP", "완료", CY),
         ("2단계", "8월 상순", "KOREN AI Cloud 구축\nAI Hub 실데이터 수신 · ETL", "완료", CY),
         ("3단계", "8월 중순", "번역·인식 모델 학습\n브라우저 배포 · 양방향 왕복", "완료 + 추가", CY_D),
         ("4단계", "8월 하순~9월", "연속 인식 · 일상어휘 편입\n당사자 평가", "일부 선행 달성", CY_D),
         ("5단계", "10월", "KOREN 실회선 다지점 송출\n실증 · 현장 시연", "예정", C(0xA9,0xB4,0xBF))]
for i, (st, when, what, state, col) in enumerate(steps):
    x = 0.72 + i * 2.418
    w = 2.274
    rect(s, x, 1.42, w, 2.05, fill=LIGHT)
    rect(s, x, 1.42, w, 0.05, fill=col)
    text(s, x + 0.22, 1.62, 1.2, 0.28, [(st, 12.5, True, INK)])
    text(s, x + 0.22, 1.94, w - 0.4, 0.28, [(when, 10, False, MUT)])
    text(s, x + 0.22, 2.28, w - 0.4, 0.7, [(what, 10.2, False, INK)], spacing=1.28)
    rect(s, x + 0.22, 3.02, 1.55, 0.30, fill=col)
    text(s, x + 0.22, 3.06, 1.55, 0.26, [(state, 9.5, True, WHITE)], align=PP_ALIGN.CENTER)
    if i < 4:
        text(s, x + w, 2.25, 0.144, 0.3, [("›", 15, True, C(0xC3,0xCB,0xD4))], align=PP_ALIGN.CENTER)
rect(s, 0.72, 3.78, 5.63, 3.12, fill=C(0xEC,0xF7,0xF9))
text(s, 1.02, 3.98, 5.1, 0.32, [("3단계에서 계획에 없던 것도 했습니다", 12.5, True, CY_D)])
extra = ["서비스워커 오프라인 동작. 회선이 끊겨도 열린다",
         "창구 대화 화면. 직원 음성을 수어로, 농인의 답을 음성으로",
         "재난별 행동요령 수어 · 긴급 SOS 전체화면",
         "숫자 수어(수 읽기) · 표현 못 한 낱말은 낱말 카드로",
         "묻기 화면. 수어 질의응답과 타일 없는 방향 지도"]
for i, e in enumerate(extra):
    y = 4.44 + i * 0.46
    rect(s, 1.02, y + 0.08, 0.07, 0.07, fill=CY, shape=MSO_SHAPE.OVAL)
    text(s, 1.22, y, 4.95, 0.3, [(e, 10.5, False, INK)])
rect(s, 7.0, 3.78, 5.63, 3.12, fill=LIGHT)
text(s, 7.3, 3.98, 5.1, 0.32, [("어떻게 함께 일했나", 12.5, True, INK)])
how = [("판단을 기록으로 남긴다",
        "기술 판단은 DECISIONS.md에 근거와 함께 적습니다. 왜 안 붙였는지까지 남깁니다."),
       ("수치는 사람이 아니라 스크립트가 낸다",
        "발표하는 숫자는 저장소의 측정 스크립트가 다시 계산합니다."),
       ("바꿀 때마다 자동 검사를 돌린다",
        "폰·태블릿·키오스크 실조작까지 포함해 매 변경마다 실행합니다."),
       ("회의는 기록으로 남긴다",
        "킥오프, 데이터 전략, 중간평가 대비까지 회의록 3건.")]
for i, (t, d) in enumerate(how):
    y = 4.44 + i * 0.63
    rect(s, 7.3, y + 0.06, 0.07, 0.07, fill=CY_D, shape=MSO_SHAPE.OVAL)
    text(s, 7.5, y - 0.02, 4.9, 0.28, [(t, 11, True, INK)])
    text(s, 7.5, y + 0.26, 4.9, 0.3, [(d, 9.8, False, MUT)])
footer(s, "심사항목 · 개발과정 적정성 30점")

# ═══════════════════════════════════════════ 멘토 의견 반영
s = sld(); header(s, "멘토 의견을 받아 설계를 바꿨습니다",
                  "들은 것과 바꾼 것을 하나씩 대응시켜 적었습니다")
text(s, 0.72, 1.28, 2.35, 0.26, [("언제 · 누구에게", 10, True, MUT)])
text(s, 3.20, 1.28, 4.20, 0.26, [("받은 의견", 10, True, MUT)])
text(s, 7.55, 1.28, 5.08, 0.26, [("바꾼 것", 10, True, CY_D)])
rect(s, 0.72, 1.54, 11.91, 0.014, fill=LINE)
mentor = [("7월 23일", "KOREN 자원 활용 미팅",
           "AI Cloud·HPC·VM을 어떻게 신청하고 쓰는지, 테스트베드는\n과제 기간 중 반드시 활용할 것",
           "자원별 역할을 갈랐습니다. AI Cloud는 학습만, HPC는 CPU 전처리,\nKOREN VM은 상시 데모. 하나로 다 하려던 계획을 접었습니다."),
          ("8월 5일", "HPC 이노베이션 허브 교육",
           "VM 구성, PBS Torque 스케줄러, 분산 작업 제출 방법",
           "영상 → 랜드마크 변환을 HPC로 분리하는 분산 설계를 확정하고,\n샤딩 옵션과 PBS 잡 스크립트를 저장소에 넣었습니다."),
          ("8월 7일", "KOREN AI Cloud 교육",
           "GPU 워크스페이스·볼륨 운영, MIG의 연산 특성",
           "볼륨 운영 원칙을 세우고(홈은 패키지 전용), MIG 연산 제약에 맞춰\n영상 CNN 대신 랜드마크 트랜스포머 구조를 골랐습니다."),
          ("지도교수 멘토링", "컴퓨터공학",
           "평가 수치의 신뢰성을 확보할 것.\n실데이터 기반 검증, 실시간성과 경량화",
           "수어자 분리 평가를 표준으로 못 박고, 학습에 없던 수어자 13명으로\n이중 엄격 평가를 더했습니다(78.6%). 브라우저 단독 추론으로 경량화."),
          ("지도교수 멘토링", "사회복지학",
           "농인의 한국어 문해력 편차를 고려할 것.\n텍스트 중심 화면을 지양하고 실제 당사자 검증이 필요",
           "한국어를 최소화한 당사자 전용 화면을 만들고, 복지관 두 곳에서\n직접 시연·관찰했습니다(05–06쪽).")]
for i, (when, who, said, did) in enumerate(mentor):
    y = 1.62 + i * 1.06
    rect(s, 0.72, y, 11.91, 0.96, fill=LIGHT if i % 2 == 0 else WHITE)
    rect(s, 0.72, y, 0.045, 0.96, fill=CY if i < 3 else CY_D)
    text(s, 1.0, y + 0.16, 2.1, 0.28, [(when, 11, True, INK)])
    text(s, 1.0, y + 0.46, 2.1, 0.28, [(who, 9.5, False, MUT)])
    text(s, 3.20, y + 0.20, 4.15, 0.62, [(said, 10, False, MUT)], spacing=1.28)
    text(s, 7.55, y + 0.20, 5.03, 0.62, [(did, 10, False, INK)], spacing=1.28)
footer(s, "심사항목 · 개발과정 적정성(멘토링 반영) 10점")

# ═══════════════════════════════════════════ 한계
s = sld(); header(s, "아직 안 되는 것을 먼저 말씀드립니다",
...
limits = [("이어서 하는 수어", "학습 완료 · 미적용",
           ...
           "농인이 화장실을 물어도 앱이 영영 못 알아듣게 되는데 오류도 경고도 나지 않습니다.", AMB),
...
for i, (when, t, items, col) in enumerate(phases):
    x = 0.72 + i * 4.06
    rect(s, x, 1.55, 3.85, 3.80, fill=LIGHT)
...
rect(s, 0.72, 5.62, 11.9, 1.05, fill=C(0xEC,0xF7,0xF9))
text(s, 1.02, 5.62, 11.9, 1.05,
     [("우선순위는 ‘정확도’가 아니라 ‘틀렸을 때의 피해’로 정합니다. "
       "잘못된 수어는 표현되지 않은 것보다 나쁩니다. 농인은 그것을 믿기 때문입니다.", 12.5, True, INK)],
     align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
footer(s)

# ═══════════════════════════════════════════ 기대효과
s = sld(); header(s, "기대효과", "재난 상황의 정보 접근권 공백을 기술로 메웁니다")
imgcrop(s, "web_기대효과.png", 0.72, 1.45, 6.0, top=0.167, bot=0.100)
text(s, 0.72, 4.42, 6.0, 0.32, [("구현 난이도. 무엇이 어려웠나", 12.5, True, INK)])
hard = [("학습과 추론이 같은 특징을 봐야 한다",
         "어긋나면 검증 95%가 웹캠에선 0%가 됩니다. 현재 오차 0."),
        ("연산이 부족한 GPU 안에서 13,576클래스",
         "MIG는 H200의 약 29% 연산입니다. 영상 CNN을 포기하고 2.8M 랜드마크 트랜스포머로 풀었습니다."),
        ("서버 없이 브라우저 안에서 전부",
         "번역 30MB · 인식 20MB를 int8로 줄여 넣고, 통신이 끊겨도 그대로 돕니다.")]
for i, (t, d) in enumerate(hard):
    y = 4.80 + i * 0.72
    rect(s, 0.72, y, 0.045, 0.62, fill=AMB)
    text(s, 1.0, y - 0.02, 5.7, 0.28, [(t, 11, True, INK)])
    text(s, 1.0, y + 0.26, 5.7, 0.34, [(d, 9.5, False, MUT)], spacing=1.22)
...
rect(s, 7.05, 5.55, 5.58, 1.32, fill=BG_D)
text(s, 7.32, 5.76, 5.05, 0.95,
     [("확장 가능성", 11.5, True, CY),
      ("같은 구조로 병원·주민센터 창구, 공공 키오스크, 교육 현장에\n"
       "적용할 수 있습니다. 서버가 필요 없어 기관마다 별도 인프라\n없이 배포됩니다.", 10.5, False, C(0xD5,0xDD,0xE5))], spacing=1.3)
footer(s, "심사항목 · 기대효과(구현 난이도 포함) 20점")
...
text(s, 1.3, 6.15, 11, 0.4, [("감사합니다.", 17, True, C(0xD5,0xDD,0xE5))])
text(s, 1.3, 6.62, 11, 0.35,
     [("닿다 · KOREN 저지연망 기반 재난정보 실시간 AI 수어(KSL) 통역·송출 서비스", 11, False, C(0x6E,0x7A,0x88))])

SHDW = (
    '<a:effectLst xmlns:a="%s"><a:outerShdw blurRad="88900" dist="22225" '
    'dir="5400000" rotWithShape="0"><a:srgbClr val="0B1220">'
    '<a:alpha val="8000"/></a:srgbClr></a:outerShdw></a:effectLst>' % A)


def soften(sh):
    """그림자는 카드에만. 머리·바닥 띠와 통짜 배경은 건드리지 않는다."""
    spPr = sh._element.spPr
    old = spPr.find(qn("a:effectLst"))
    if old is not None:
        spPr.remove(old)
    spPr.append(parse_xml(SHDW))
...
for _sl in prs.slides:
    for _sh in _sl.shapes:
        if _sh.shape_type != MSO_SHAPE_TYPE.AUTO_SHAPE:
            continue
        if _sh.left < I(0.5) or _sh.top < I(1.05):
            continue
        if _sh.width < I(1.2) or _sh.height < I(0.5) or _sh.width > I(12.2):
            continue
        try:
            rgb = _sh.fill.fore_color.rgb
        except Exception:
            continue
        if lum(rgb) >= 0.88 or rgb == BG_D:
            soften(_sh)

# 제출 안내문의 파일명 규칙: 중간평가 발표자료_팀명 (운영계획 붙임2 기준 팀명 = 닿다)
out = Path("ppt/중간평가 발표자료_닿다.pptx")
prs.save(str(out))
