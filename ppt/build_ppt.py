...
text(s, 1.3, 2.05, 11, 0.4, [("K-디지털 챌린지 · 넷 챌린지 캠프 시즌 13 · 중간평가", 14, True, CY)])
text(s, 1.3, 2.62, 11.2, 1.9,
     [("들리지 않아도,", 50, True, WHITE), ("닿습니다", 50, True, CY)], spacing=1.12)
text(s, 1.3, 4.62, 10.5, 0.9,
     [("한국수어(KSL) 양방향 실시간 번역 시스템 — SignBridge", 19, True, C(0xDA,0xE2,0xEA))])
rect(s, 1.3, 5.32, 3.2, 0.035, fill=CY)
text(s, 1.3, 5.58, 11, 0.9,
     [("재난 정보를 수어로 전달하고, 농인의 수어를 한국어와 음성으로 되돌린다.", 13.5, False, C(0x9A,0xA6,0xB4)),
      ("서버 없이 브라우저에서 동작 · 통신이 끊긴 재난 상황에서도 작동", 13.5, False, C(0x9A,0xA6,0xB4))], spacing=1.35)
text(s, W - 5.3, H - 1.15, 4.7, 0.4, [("2026. 08. 19.", 12, False, C(0x6E,0x7A,0x88))], align=PP_ALIGN.RIGHT)

# ═══════════════════════════════════════════ 02 목차
s = sld(); header(s, 2, "발표 순서")
cols = [
    ("문제와 목표", "왜 이 문제를 푸는가",
     ["03  재난 정보 전달의 공백", "04  시스템 개요", "05  4-에이전트 구조"]),
    ("핵심 기술", "두 방향을 어떻게 만들었나",
     ["06  한국어 → 수어", "07  수어 → 한국어·음성", "08  학습 데이터"]),
    ("검증과 결과", "무엇을, 어떻게 쟀나",
     ["09  인식 정확도", "10  번역 품질", "11  자동 검증 체계"]),
    ("화면·현장·계획", "실제로 써 보고 받은 것",
     ["12–13  당사자 화면", "14–15  현장 방문", "16–18  한계와 다음 단계"]),
]
for i, (t, sub, items) in enumerate(cols):
    x = 0.62 + i * 3.13
    rect(s, x, 1.62, 2.92, 3.02, fill=LIGHT)
    rect(s, x, 1.62, 2.92, 0.05, fill=CY)
    text(s, x + 0.28, 1.92, 2.4, 0.4, [(t, 15.5, True, CY_D)])
    text(s, x + 0.28, 2.36, 2.5, 0.3, [(sub, 9.8, False, MUT)])
    rect(s, x + 0.28, 2.72, 0.6, 0.022, fill=LINE)
    for j, it in enumerate(items):
        text(s, x + 0.28, 2.94 + j * 0.5, 2.5, 0.4, [(it, 11.5, False, INK)], spacing=1.2)
...
# ═══════════════════════════════════════════ 03 문제
s = sld(); header(s, 3, "재난 정보는 농인에게 도착하지 않습니다",
...
# ═══════════════════════════════════════════ 04 시스템 개요
s = sld(); header(s, 4, "시스템 개요", "서버 없이 브라우저 안에서 두 방향이 모두 동작합니다")
...
# ═══════════════════════════════════════════ 05 4-에이전트
s = sld(); header(s, 5, "4개의 에이전트가 이어서 일합니다", "각 단계의 결과를 기록해, 조용한 실패를 검사로 잡아냅니다")
...
# ═══════════════════════════════════════════ 06 한국어 → 수어
s = sld(); header(s, 6, "핵심기술 ① 한국어를 수어로", "문장 → 글로스 열 → 실제 수어자의 키포인트를 이어 재생")
...
# ═══════════════════════════════════════════ 07 수어 → 한국어
s = sld(); header(s, 7, "핵심기술 ② 수어를 한국어와 음성으로", "웹캠 → 관절 좌표 → 낱말 인식 → 문장 → 음성. 영상은 기기 밖으로 나가지 않습니다")
...
# ═══════════════════════════════════════════ 08 데이터
s = sld(); header(s, 8, "학습 데이터", "AI Hub 수어 말뭉치 — 원본을 직접 열어 확인하며 가공했습니다")
...
# ═══════════════════════════════════════════ 09 인식 정확도
s = sld(); header(s, 9, "검증 ① 수어 낱말 인식 정확도", "처음 보는 수어자에서도 검증치와 같은 성능이 나왔습니다")
...
# ═══════════════════════════════════════════ 10 번역 품질
s = sld(); header(s, 10, "검증 ② 번역 품질", "표현률은 “나갔는가”만 잽니다 — 맞게 나갔는지는 따로 쟀습니다")
...
# ═══════════════════════════════════════════ 11 자동 검증
s = sld(); header(s, 11, "검증 ③ 자동 검증 체계", "이 앱의 실패는 대부분 “화면은 정상인데 알맹이가 없는” 모양입니다")
...
# ═══════════════════════════════════════════ 12 앱 화면
s = sld(); header(s, 12, "당사자 화면 — 네 개의 탭", "농인이 실제로 쓰는 화면입니다")
...
# ═══════════════════════════════════════════ 13 묻기 + 오프라인
s = sld(); header(s, 13, "농인이 ‘묻는’ 쪽이 됩니다", "통신이 끊긴 상태에서도 위치 기반으로 답합니다")
...
# ═══════════════════════════════════════════ 14 현장 방문 (건물 + 개요)
s = sld(); header(s, 14, "현장 방문 — 두 곳의 복지관에서 직접 확인했습니다",
...
# ═══════════════════════════════════════════ 15 시연과 피드백 (사진 4)
s = sld(); header(s, 15, "실제 이용을 돕고, 개선점을 받았습니다",
...
# ═══════════════════════════════════════════ 16 한계
s = sld(); header(s, 16, "아직 안 되는 것 — 먼저 말합니다",
...
# ═══════════════════════════════════════════ 17 다음 단계
s = sld(); header(s, 17, "다음 단계", "중간평가 이후 최종까지")
phases = [("9월", "농인 당사자 평가",
...
# ═══════════════════════════════════════════ 18 기대효과
s = sld(); header(s, 18, "기대효과", "재난 상황의 정보 접근권을 기술로 메웁니다")
