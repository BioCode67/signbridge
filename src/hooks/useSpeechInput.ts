 ...
 * 우리 서버는 필요 없다 — 붙일 백엔드가 없다는 뜻이다.
 *
 * **다만 이것은 온디바이스가 아니다.** Web Speech API는 브라우저가 오디오를
 * 제조사 클라우드로 보내 인식한다(Chrome은 구글). 이 앱에서 **인터넷 없이 못 쓰는
 * 유일한 기능**이고, 목소리가 우리 손 밖으로 나가는 유일한 자리이기도 하다.
 * (카메라 영상은 다르다 — MediaPipe·ONNX가 브라우저 안에서 돌아 전송되지 않는다.)
 * TalkMode가 오프라인일 때 "마이크는 안 되지만 카드·글쓰기·수어는 됩니다"로 안내한다.
 *
 * 알아둘 것
